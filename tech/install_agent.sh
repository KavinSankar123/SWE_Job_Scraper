#!/usr/bin/env bash
# -----------------------------------------------------------------------------
# install_agent.sh — run the TECH watcher (mid-level SWE roles) in the background,
# with no terminal open, using macOS launchd.
#
...
#
#   ./tech/install_agent.sh install [--interval-hours N] [--no-seed]   # default 3h
#   ./tech/install_agent.sh status
#   ./tech/install_agent.sh uninstall
#   ./tech/install_agent.sh run-now      # trigger one pass immediately
# -----------------------------------------------------------------------------
set -euo pipefail

REPO="$(cd "$(dirname "$0")/.." && pwd)"
LABEL="com.kavin.techwatcher"
PLIST="$HOME/Library/LaunchAgents/$LABEL.plist"
TEMPLATE="$REPO/tech/com.kavin.techwatcher.plist.example"
LAUNCHER="$REPO/run_tech.sh"
DOMAIN="gui/$(id -u)"
...
install)
  ...
  # Without this the agent would fail silently every few hours forever.
  if [ ! -f "$LAUNCHER" ]; then
  ...
  if grep -q 'xxxx xxxx xxxx xxxx' "$LAUNCHER"; then
    echo "ERROR: run_tech.sh still has the placeholder app password."
  ...
  case "$REPO/" in
    "$HOME"/Downloads/*|"$HOME"/Documents/*|"$HOME"/Desktop/*)
      ...
      echo "         ./tech/install_agent.sh install --interval-hours $interval_hours"
      echo
  ...
  # --- seed the store BEFORE the agent starts emailing ---------------------- #
  # The tech watcher seeds silently on its first run, but that pass finds ~1,450
  # roles and takes a while. Doing it here means the first launchd fire is a
  # normal incremental pass, and you see the seed happen instead of wondering
  # why the agent is quiet.
  if [ "$seed" -eq 1 ] && [ ! -f "$REPO/tech/seen_tech_jobs.sqlite3" ]; then
    echo "No store yet — seeding it now so the agent doesn't email you a huge backlog."
    echo "This sweeps every board and takes a few minutes. Nothing is emailed."
    "$LAUNCHER" --once || { echo "ERROR: seeding run failed; not installing."; exit 1; }
    echo
  fi
  ...
  echo "  plist : $PLIST"
  echo "  logs  : $REPO/launchd.tech.out.log"
  echo
  ...
  echo "Watch the first run with:  tail -f $REPO/launchd.tech.out.log"
  ;;
...
status)
  if [ ! -f "$PLIST" ]; then
    ...
    echo "Install it with: ./tech/install_agent.sh install"
    exit 0
  fi
  echo "plist: $PLIST"
  ...
  tail -n 5 "$REPO/launchd.tech.out.log" 2>/dev/null | sed 's/^/  /' || echo "  (no log yet)"
  ;;

run-now)
  launchctl kickstart -k "$DOMAIN/$LABEL"
  echo "Triggered one pass. Follow it with:"
  echo "  tail -f $REPO/launchd.tech.out.log"
  ;;
